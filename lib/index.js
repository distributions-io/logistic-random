...
*	Creates a matrix or array filled with logistic random numbers.
*
...
function random( dims, options ) {
	...
	s = typeof opts.s !== 'undefined' ? opts.s : 1;
	if ( arguments.length === 0 ) {
		return number( mu, s, rand );
	}
	if ( isArray ) {
		ndims = dims.length;
		if ( ndims < 2 ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, mu, s, rand );
		}
		return typedarray( len, dt, mu, s, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, mu, s, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, mu, s, rand );
} // end FUNCTION random()
